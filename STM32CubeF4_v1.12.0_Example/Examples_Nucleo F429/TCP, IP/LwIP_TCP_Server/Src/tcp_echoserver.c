...
void tcp_echoserver_init(void)
{
  ...
  if (tcp_echoserver_pcb != NULL)  {
		err_t  err;
		// -- <4-2> 위에서 만든 tcp_echo_pcb에  접속될 client의 IP ( IP_ADDR_ANY)와
		//             Port (PORT)를 bind 함
...
// -------------------------------------------------------------------------
//  -- <12>  F107 Board(Server)에서  PC(Client)로 메시지를 전송하는 함수
//
// -------------------------------------------------------------------------
...
void HAL_GPIO_EXTI_Callback(uint16_t GPIO_Pin)
{
  ...
  // -- <13-1> 눌러지는 Key 값에 따라 message_send( ) 함수를 호출함		
